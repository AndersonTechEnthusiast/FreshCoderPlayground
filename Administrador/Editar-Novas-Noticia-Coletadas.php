...
<?php
    ...
    if($_SESSION['Administrador']['Login'] == true && $_SESSION['Administrador']['nivel_acessos'] == 'Administrador')
    {
        ...
        {
            $newNamesEditionPost = ['ID','Novo_Título', 'Nova_Categoria', 'Nova_Descrição'];
            $ValuesEditionPost = array_values($_POST);
            $Novos_Dados = [];
            $count = count($_POST);
            for($i = 0; $i < $count; $i++)
            {
                $Novos_Dados[$newNamesEditionPost[$i]] = $ValuesEditionPost[$i];
            }
            try
            {
                $host = 'localhost';
                ...
                if($conn)
                {
                    $sql = "UPDATE postagens SET titulo = :titulo , descricao = :descricao , categoria = :categoria WHERE id_postagem = :id_postagem";
                    $stmt = $conn->prepare($sql);
                    $stmt->bindValue(':titulo',$Novos_Dados['Novo_Título']);
                    $stmt->bindValue(':descricao',$Novos_Dados['Nova_Descrição']);
                    $stmt->bindValue(':categoria',$Novos_Dados['Nova_Categoria']);
                    $stmt->bindValue(':id_postagem',$Novos_Dados['ID']);
                    if($stmt->execute())
                    {
                        die
                        (
                            '<script>
                                Swal.fire({
                                    icon: "success",
                                    title: "Informações Atualizadas Pelo Administrador",
                                    text: "As Informações da Postagem foram Atualizadas pelo Administrador com Sucesso !!!",
                                    didClose: () => {
