...
<?php
    ...
    if($_SESSION['Administrador']['Login'] == true && $_SESSION['Administrador']['nivel_acessos'] == 'Administrador')
    {
        ...
        {
            $newNamesEditionUser = ['ID','Novo_Nivel_de_Acessos'];
            $ValuesEditionUser = array_values($_POST);
            $Novos_Dados = [];
            $count = count($_POST);
            for($i = 0; $i < $count; $i++)
            {
                $Novos_Dados[$newNamesEditionUser[$i]] = $ValuesEditionUser[$i];
            }
            try
            {
                $host = 'localhost';
                        ...
                        (
                            '<script>
                                Swal.fire({
                                    ...
                                    text: "O Nivel de Acesso do Usuario foi Atualizado com Sucesso !!!",
                                    didClose: () => {
